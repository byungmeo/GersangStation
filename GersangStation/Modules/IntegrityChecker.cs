...
public class IntegrityChecker {
    ...
    private void WriteFullClientFile(string path) {
        ...
        if(_verbose) Trace.WriteLine("Header request");
        try {
            using (HttpClient client = new HttpClient())
            {
                ...
                if(_verbose) Trace.WriteLine($"Size of Full Client = {fullSizeString}");
        ...
        if(_verbose) Trace.WriteLine($"succeeded. file size = {fullSizeString}");
        ...
        if(_verbose) Trace.WriteLine("Footer request");
        try {
            using(HttpClient client = new HttpClient()) {
                //Make request 
                HttpRequestMessage requestMessage = new HttpRequestMessage(HttpMethod.Get, FullClientInfo.Url);
        ...
        if(fullSize != fileInfo.Length) throw new InvalidDataException(message);
    }
    ...
    private Dictionary<string, string> GetFileListFromVersion(int version) {
        ...
        using(HttpClient client = new HttpClient()) {
            ...
            if(_verbose) Trace.WriteLine($"Version {version}: ");
    ...
    private List<int> GetVersionListFromReadMe() {
        ...
        using (HttpClient client = new HttpClient()) {
            ...
            if(readmeFile == null) throw new Exception("Cannot read readme file");
            //if(_verbose) {
            ...
            foreach(var version in versions) {
                ...
                if(_verbose) Trace.Write($"{ver} -> ");
                if(ver.Contains("패치") == false) {
                    if(_verbose) Trace.WriteLine("");
                    continue;
                } 
                try {
    ...
    public Dictionary<string, string>? GetFullClientFileListFromWeb(out int version) {
        ...
        if(_verbose) Trace.WriteLine($"Run {SevenZipPath} {args}");
