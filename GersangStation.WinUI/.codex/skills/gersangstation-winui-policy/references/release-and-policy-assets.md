...
- Use one elevated scheduled task for both admin startup and admin desktop shortcuts; startup only controls whether that task has a logon trigger, and scheduler support files must live under `%LOCALAPPDATA%\GersangStation\AdminStartup` instead of package-scoped `LocalState`.
