...
- Use one elevated scheduled task for both admin startup and admin desktop shortcuts; startup is always administrator launch and only controls whether that task has a logon trigger. Scheduler support files must live under `%LOCALAPPDATA%\GersangStation\AdminStartup` instead of package-scoped `LocalState`.
