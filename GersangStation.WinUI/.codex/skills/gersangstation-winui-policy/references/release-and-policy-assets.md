...
- Package the app with `allowElevation` and `requireAdministrator` so normal shell launches can use the app manifest for elevation.
- Create desktop shortcuts as normal AppsFolder links to the packaged app; do not route shortcuts through Task Scheduler.
- Register Windows logon startup through the packaged `desktop:StartupTask` manifest extension and `StartupTask` API so it appears in Windows Startup Apps. Do not create Task Scheduler tasks or support scripts for normal shortcut or startup registration.
