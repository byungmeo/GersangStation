...
public static class PatchPlanBuilder_StringRows
{
    ...
    /// row[6] = compressed file CRC(첫 번째 엔트리 CRC32, 없을 수 있음)
    ///
    /// 중복 키 = row[3] + row[1]
        ...
        IReadOnlyDictionary<int, List<string[]>> entriesByVersion)
    {
        ...
        foreach (var kv in entriesByVersion.OrderBy(kv => kv.Key)) // 오름차순
        {
            ...
            foreach (var row in rows)
            {
                ...
                string comp = row[1];
                string relDir = row[3];
                string checksum = row.Length > 6 ? row[6] : (row.Length > 4 ? row[4] : string.Empty);
                ...
                latestByKey[key] = new PatchFile(
                    ...
                    ArchiveChecksum: string.IsNullOrWhiteSpace(checksum) ? null : checksum.Trim());
            }
        }
