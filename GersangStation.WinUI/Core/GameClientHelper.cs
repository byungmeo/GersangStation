...
public static class GameClientHelper
{
    ...
    */
    ...
    private static bool CreateSymbolClient(string orgInstallPath, string destPath, bool overwriteConfig, out string reason)
    {
        reason = string.Empty;
        ...
        if (Directory.Exists(charPath) && !IsSymbolDirectory(charPath))
        {
        ...
        // Online 정책
        {
            ...
            foreach (var orgFilePath in Directory.GetFiles(orgOnlinePath))
            {
                ...
                bool overwrite = !isConfig || overwriteConfig;
    ...
    public static bool CreateSymbolMultiClient(CreateSymbolMultiClientArgs args, out string reason)
    {
        ...
        {
            bool success = CreateSymbolClient(args.InstallPath, args.DestPath2, args.OverwriteConfig, out reason);
            if (!success)
            {
        ...
        {
            bool success = CreateSymbolClient(args.InstallPath, args.DestPath3, args.OverwriteConfig, out reason);
            if (!success)
            {
