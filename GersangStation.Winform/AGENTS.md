...
- At startup, `MiniUpdaterMaintenance` silently checks the updater embedded in the main app and installs a newer version in background. Its 30-second budget and app-exit cancellation must not delay startup/shutdown or display UI. Keep updater versioning in its own project; increase it when its behavior changes. Successful updater runs display a 10-second countdown and close automatically.
