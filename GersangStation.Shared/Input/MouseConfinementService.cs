...
/// </summary>
public sealed class MouseConfinementService : IDisposable
{
    ...
    private static readonly ConcurrentDictionary<nint, LowLevelMouseProc> ActiveHooks = new();
    private readonly IProgress<MouseConfinementDiagnostic> _diagnostics;
    private readonly int _ownerThreadId = Environment.CurrentManagedThreadId;
    private bool _allowAltEscape = true;
    private bool _onlyFirstWindow;
    private nint _firstWindow;
    ...
    /// <summary>
    /// Creates a disabled service on the host message-loop thread. Diagnostics must report asynchronously.
    /// </summary>
    public MouseConfinementService(IProgress<MouseConfinementDiagnostic> diagnostics)
    {
    ...
    /// This state-only operation is thread-safe so a window-switch monitor can suspend immediately.
    /// </summary>
    ...
    /// <summary>Returns the effective monitoring state, including background failure shutdown.</summary>
    public bool IsEnabled { get { lock (_syncRoot) return _isEnabled && !_isDisposed; } }

    /// <summary>Configures legacy options; WinUI uses the defaults (Alt enabled, all foreground game windows).</summary>
    public void ConfigureCompatibility(bool allowAltEscape, bool onlyFirstWindow)
    {
        VerifyOwnerThread();
        lock (_syncRoot)
        {
            ObjectDisposedException.ThrowIf(_isDisposed, this);
            _allowAltEscape = allowAltEscape;
            if (_onlyFirstWindow != onlyFirstWindow)
            {
                _firstWindow = 0;
                _firstProcessId = 0;
            }
            _onlyFirstWindow = onlyFirstWindow;
            ClearCachedTarget_NoLock();
        }
    }

    /// <summary>Clears the legacy first-window selection without changing its mode.</summary>
    public void ResetFirstWindow()
    {
        VerifyOwnerThread();
        lock (_syncRoot)
        {
    ...
    private void VerifyOwnerThread()
    {
        if (Environment.CurrentManagedThreadId != _ownerThreadId)
            throw new InvalidOperationException("Control mouse confinement from its owning message-loop thread.");
    }

    /// <summary>Contains timer exceptions at the background boundary and disables further correction.</summary>
    private void PollSafely()
    {
        try { PollCursorClip(); }
        catch (Exception exception) { StopAfterFailure(exception, "PollCursorClip"); }
    }

    private void StopAfterFailure(Exception exception, string operation)
    {
        lock (_syncRoot)
        {
            _isEnabled = false;
        ...
        ReportDiagnostic(new(operation, exception, MonitoringStopped: true));
    }

    private void ReportDiagnostic(MouseConfinementDiagnostic diagnostic)
    {
        // A faulty observer must never unwind a native hook or a ThreadPool callback.
        try { _diagnostics.Report(diagnostic); }
        catch (Exception exception) { Trace.TraceError("Mouse confinement diagnostic observer failed: {0}; original: {1}", exception, diagnostic); }
    }

    private void StartMonitor_NoLock()
    {
        InstallMouseHook_NoLock();
        _monitorTimer ??= new Timer(_ => PollSafely(), null, TimeSpan.Zero, PollInterval);
    }
    ...
    private void StopMonitor_NoLock()
    {
        ...
        {
            ReportDiagnostic(new("UnhookWindowsHookEx", new Win32Exception(Marshal.GetLastWin32Error()), MonitoringStopped: true));
            return;
        }

        ActiveHooks.TryRemove(_mouseHookHandle, out _);
    ...
    private void InstallMouseHook_NoLock()
    {
        ...
        if (_mouseHookHandle == IntPtr.Zero)
        {
            ReportDiagnostic(new("SetWindowsHookEx", new Win32Exception(Marshal.GetLastWin32Error()), MonitoringStopped: false));
        }
        else
        {
            ActiveHooks[_mouseHookHandle] = _mouseHookProcedure;
        }
    }
    ...
    {
        try { return ProcessMouseHook(code, wParam, lParam); }
        catch (Exception exception)
        {
            StopAfterFailure(exception, "MouseHookCallback");
            return CallNextHookEx(IntPtr.Zero, code, wParam, lParam);
        }
    }

    private nint ProcessMouseHook(int code, nint wParam, nint lParam)
    {
    ...
    private bool TrySetCachedTarget(WindowClipTarget target, int stateVersion)
    {
        lock (_syncRoot)
        {
            ...
            if (_onlyFirstWindow)
            {
                if (_firstWindow != 0)
                {
                    _ = GetWindowThreadProcessId(_firstWindow, out int ownerId);
                    if (ownerId != _firstProcessId || !IsWindow(_firstWindow))
                    {
                        _firstWindow = 0;
                        _firstProcessId = 0;
                    }
                ...
                if (_firstWindow == 0)
                {
                    _firstWindow = target.WindowHandle;
                    _firstProcessId = target.ProcessId;
                }
                if (_firstWindow != target.WindowHandle || _firstProcessId != target.ProcessId)
                {
                    ClearCachedTarget_NoLock();
                    return false;
                }
    ...
    private bool IsSuspendKeyPressed()
        => Volatile.Read(ref _allowAltEscape) && IsAnyKeyDown(VkMenu, VkLMenu, VkRMenu);
